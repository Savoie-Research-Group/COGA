    ...
    with open(data_file,'r') as f:
        for line in f:
            ...
            if flag == 'Masses':
                ...
                continue
            if flag == 'Atoms':
                temp_atoms[int(float(fields[0]))] = [ float(i) for i in fields[1:] ]
                if preserve_atom_order:
                    atom_ids.append(int(float(fields[0])))
                continue
            if flag == 'Bonds':
                temp_bonds[int(float(fields[0]))] = [ int(i) for i in fields[1:] ]
                if preserve_bond_order:
                    bond_ids.append(int(float(fields[0])))
                continue
            if flag == 'Angles':
                temp_angles[int(float(fields[0]))] = [ int(i) for i in fields[1:] ]                
                if preserve_angle_order:
                    angle_ids.append(int(float(fields[0])))
                continue
            if flag == 'Dihedrals':
                temp_dihedrals[int(float(fields[0]))] = [ int(i) for i in fields[1:] ]
                if preserve_dihedral_order:
                    dihedral_ids.append(int(fields[0]))
                continue
            if flag == 'Impropers':
                temp_impropers[int(float(fields[0]))] = [ int(i) for i in fields[1:] ]
                if preserve_improper_order:
                    improper_ids.append(int(float(fields[0])))
                continue
            if flag == 'Velocities':
                velocities[int(float(fields[0]))] = [ float(i) for i in fields[1:] ]
                continue
            if flag == 'Ellipsoids':
                ellipsoids[int(float(fields[0]))] = [ float(i) for i in fields[1:] ]
